...
public class AdminDao {
    ...
    public boolean login(String username,String password){
        try {
            ...
            ps.setString(1, username);// Gán giá trị username
            ps.setString(2,password);
            rs=ps.executeQuery();
            // Kiểm tra xem có kết quả nào từ cơ sở dữ liệu hay không
            if(rs.next()){
    ...
    public boolean getSecurity(String username){
        try {
            ...
            // Kiểm tra xem có kết quả nào từ cơ sở dữ liệu hay không
            if(rs.next()){
                ForgotPassword.jTextField2.setText(rs.getString(4));
                return true;
            }
        } catch (Exception ex) {
            Logger.getLogger(AdminDao.class.getName()).log(Level.SEVERE, null, ex);
